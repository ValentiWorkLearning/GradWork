...
void BatteryWidget::initStyles() noexcept
{
    ...
    m_bateryIconStyle =
        pThemeProvider->getIconsFont(Theme::FontSize::small, Theme::Color::MainThemeLight);
}
...
void BatteryWidget::resetStyle() noexcept
{
    Meta::tupleApply(
        ...
        std::forward_as_tuple(m_batteryLabelStyle, m_bateryIconStyle));
}
...
void BatteryWidget::setBatteryLevelPercentage(const std::uint8_t _newBatteryLevel) noexcept
{
    ...
    auto [p, ec] = std::to_chars(tempStr.data(), tempStr.data() + tempStr.size(), _newBatteryLevel);
...
void BatteryWidget::hide() noexcept
{
    ...
    Meta::tupleApply(
        ...
        std::forward_as_tuple(m_pBatteryIcon, m_pBatteryLabel));
}

...
void PagesSwitch::hide() noexcept
{
    ...
    Meta::tupleApply(
        ...
        std::forward_as_tuple(m_pFirstPage, m_pSecondPage, m_pThirdPage));
}
...
void PagesSwitch::setActivePage(std::string_view _pageName) noexcept
{
    ...
    else if (_pageName == Views::IHealthWatchPage::HealthPageName) // TODO migrate to labels/
                                                                   // objects
    {
...
void PagesSwitch::initStyles() noexcept
{
    ...
    const auto ThemeDark = pThemeProvider->getMainThemeColor(Theme::Color::MainThemeDark);
    const auto ThemeLight = pThemeProvider->getMainThemeColor(Theme::Color::MainThemeLight);
...
void PagesSwitch::resetStyle() noexcept
{
    Meta::tupleApply(
        ...
        std::forward_as_tuple(m_pointStyle, m_pointStyleChecked));
}
    ...
    const std::uint32_t _displayHeight) noexcept
{
    ...
    lv_obj_align(
        m_pSecondPage.get(), LV_ALIGN_BOTTOM_MID, 0, -static_cast<int>(_displayHeight / 10));

...
template <typename HeadEvent, typename... Events> class Helper : public Helper<Events...>
{
protected:
    virtual void handleEventImpl(const HeadEvent& _eventToHandle, const std::any& _eventData) = 0;
};

template <typename HeadEvent> class Helper<HeadEvent>
{

protected:
    virtual void handleEventImpl(
        const HeadEvent& _eventToHandle,
        const std::any& _eventData) noexcept = 0;
};

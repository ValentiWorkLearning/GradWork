...
void GsMainWindow::addPage(std::unique_ptr<Graphics::Views::IPageViewObject>&& _toAdd) noexcept
{
...
Graphics::Views::IPageViewObject& GsMainWindow::getActivePage() noexcept
{
    auto pPagePtr = getPagePointer(m_currentPageName);
    return *pPagePtr;
}

const Graphics::Views::IPageViewObject& GsMainWindow::getActivePage() const noexcept
{
    auto pPagePtr = getPagePointer(m_currentPageName);
    return *pPagePtr;
}

const Graphics::Views::IPageViewObject& GsMainWindow::getPage(
...
{
    m_pMainWindowView->getThemeController()->onThemeChanged.connect([this] {
...
{
    auto it = std::find_if(
        m_pagesStorage.begin(), m_pagesStorage.end(), [_pageName](const auto& _pagePair) {
...
const Graphics::Views::IPageViewObject* GsMainWindow::getPagePointer(
    std::string_view _pageName) const noexcept
{
    auto it = std::find_if(
        m_pagesStorage.begin(), m_pagesStorage.end(), [_pageName](const auto& _pagePair) {
...
const Theme::IThemeController* GsMainWindow::getThemeController() const noexcept
{
    ...
    std::unique_ptr<Graphics::Views::IPagesCreator>&& _pPagesCrator) noexcept
{
    return std::make_unique<GsMainWindow>(
        std::move(_pMainWindowView), std::move(_pWidgetsCreator), std::move(_pPagesCrator));
}

...
) noexcept
{
    ...
    if (isCoordsValid)
    {
        ...
        co_await sendCommandImplFast(yAxisCommand.data());
        co_await sendChunkFast(yAxisCommand.data()+CmdSize, ArgSize);

        static std::uint8_t RamWriteCmd{0x2C};

        co_await sendCommandImplFast(&RamWriteCmd);  //LCD_WriteCMD(GRAMWR);

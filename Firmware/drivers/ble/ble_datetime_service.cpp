#include "inc/ble/nordic_ble/ble_datetime_service.hpp"
...
void DateTimeServiceNordic::initService(const std::any& _pGattQueue) noexcept
{
    ...
    auto currentTimeServiceEventHandler =
        cbc::obtain_connector([this](ble_cts_c_t* _currentTimeService, ble_cts_c_evt_t* _pEvent) {
...
void DateTimeServiceNordic::serviceEventHandler(
    ble_cts_c_t* _pCurrentTimeService,
    ble_cts_c_evt_t* _pEvent) noexcept
{
    ret_code_t errorCode{};
    ...
    case BLE_CTS_C_EVT_DISCOVERY_COMPLETE:
        LOG_DEBUG("Current Time Service discovered on server.");
        errorCode = ble_cts_c_handles_assign(
            &m_currentTimeService, _pEvent->conn_handle, &_pEvent->params.char_handles);
        APP_ERROR_CHECK(errorCode);
        break;

    case BLE_CTS_C_EVT_DISCOVERY_FAILED:
        LOG_DEBUG("Current Time Service not found on server. ");
    ...
    case BLE_CTS_C_EVT_CURRENT_TIME:
        LOG_DEBUG("Current Time received.");
        // current_time_print(_pEvent);
        break;

    case BLE_CTS_C_EVT_INVALID_TIME:
        LOG_DEBUG("Invalid Time received.");
        break;
...
void DateTimeServiceNordic::handleDiscoveryEvent(const std::any& _pBleDbDiscoveryEvent) noexcept
{

...
{
    NRF_SDH_BLE_OBSERVER(m_serviceEvent, ServiceObserverPriority, serviceBleEventHandler, this);
}
...
void CustomService::initCustomService() noexcept
{
    ...
    std::memcpy(&baseUuid, CustomServiceUuid.data(), CustomServiceUuid.size());
    ...
    errCode = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &bleUuid, &m_serviceHandle);

#include "inc/ble/nordic_ble/ble_battery_service.hpp"
...
void BatteryLevelService::onBatteryLevelChanged(std::uint8_t _newBatteryLevel) noexcept
{
    ...
    errorCode = ble_bas_battery_level_update(&m_bas, levelToSet, BLE_CONN_HANDLE_ALL);

    if ((errorCode != NRF_SUCCESS) && (errorCode != NRF_ERROR_INVALID_STATE) &&
        (errorCode != NRF_ERROR_RESOURCES) && (errorCode != NRF_ERROR_BUSY) &&

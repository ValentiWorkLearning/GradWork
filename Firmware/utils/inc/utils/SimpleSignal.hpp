...
struct ExecuteLaterPool
{
    ...
    auto pushToLater(Function&& function, Args&&... _argList)
    {
        executionQueue.push_back(
            [this, localFunction = function, _argList...] { localFunction(_argList...); });
    }
...
private:
    ...
    bool remove_cb(size_t id)
    {
        auto it = std::remove_if(
            begin(callback_list_), end(callback_list_), [id](const CallbackSlot& slot) {
...
public:
    ...
    size_t connect(const CbFunction& cb, size_t connectionId = std::numeric_limits<size_t>::max())
    {
        return add_cb(cb, connectionId);
    }
    /// Operator to remove a signal handler through it connection ID, returns if a handler was
    /// removed.
    ...
    {
        ExecuteLaterPool::Instance().pushToLater([this, args...] { emit(args...); });
    }
 ...
 * The callbacks of a signal are invoked with the emit() method and arguments according to the
 * signature. The result returned by emit() depends on the signal collector class. By default, the
 * result of the last callback is returned from emit(). Collectors can be implemented to accumulate
 * callback results or to halt a running emissions in correspondance to callback results. The signal
 * implementation is safe against recursion, so callbacks may be removed and added during a signal
 * emission and recursive emit() calls are also safe. The overhead of an unused signal is
 * intentionally kept very low, around the size of a single pointer. Note that the Signal template
 * types is non-copyable.
 */
template <
...
/// This function creates a std::function by binding @a object to the member function pointer @a
/// method.
...
/// CollectorVector returns the result of the all signal handlers from a signal emission in a
/// std::vector.

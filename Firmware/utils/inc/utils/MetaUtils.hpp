...
constexpr auto tupleApply(Callable&& _toCall, const std::tuple<ArgsList...>& _tuple)
{
    return tupleApplyImpl(
        std::forward<Callable>(_toCall), _tuple, std::index_sequence_for<ArgsList...>{});
}
...
template <typename Unused> constexpr void UnuseVar(Unused&& _toUnuse)
{
...
template <typename ToDetect, typename Args> struct HasType
{
    static int const value = std::is_same_v<ToDetect, typename Args::Head> ||
                             HasType<ToDetect, typename Args::Tail>::value;
};

template <typename ToDetect> struct HasType<ToDetect, TypeList<>>
{
